...
public class NodeController : MonoBehaviour
{
    ...
    public void ExploreNode(GameObject Sender)
    {
        ...
        for (int i = 0; i < e.EventOptions.Count; i++)
        {
            ...
            button.transform.position = new Vector3(buttoncont.position.x - 200, buttoncont.position.y - button.transform.localScale.y * 55 * i);
            string buttonText = "";
            int numCon = 0;
            string btx = "";
            XmlDocument xmlDoc = new XmlDocument();
           ...
           // XmlDocument xmlDoc = new XmlDocument();
            ...
            foreach (KeyValuePair<Piece, int> pair in e.EventOptions[tempint].Conditions)
            {
                for(int k =0; k < pair.Value; k++)
                {
                    ...
                    resourceIcon.transform.localPosition = new Vector3(515f + 65f * (conditionCount), - button.transform.localScale.y * 55 * i);
